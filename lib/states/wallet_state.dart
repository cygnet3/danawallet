...
class WalletState extends ChangeNotifier {
  ...
  List<int> generateEntropyFromAddress(String address) {
  ...
  String generateRandomDanaAddress({required String address, required int offset}) {
    final entropy = generateEntropyFromAddress(address);
    ...
    final word1Index = (entropy[offset] << 8 | entropy[offset + 1]) % wordlistSize;
    final word2Index = (entropy[offset + 2] << 8 | entropy[offset + 3]) % wordlistSize;
    final numericValue = (entropy[offset + 4] << 8 | entropy[offset + 5]) % 1000;
  ...
  Future<String?> generateAvailableDanaAddress({
    required NameServerRepository nameServerRepository,
    required int maxRetries,
  }) async {
    for (int attempt = 0; attempt < maxRetries; attempt++) {
      final username = generateRandomDanaAddress(
        address: address,
        offset: attempt * 6, // Each attempt uses 6 bytes (2 bytes per word/number)
      );
