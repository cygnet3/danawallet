...
class _DanaAddressSetupScreenState extends State<DanaAddressSetupScreen> {
  ...
  void _onUsernameChanged(
      String value, NameServerRepository nameServerRepository) {
  ...
  Future<void> _checkAvailability(
      String username, NameServerRepository nameServerRepository) async {
    setState(() {
    ...
    try {
      final isAvailable =
          await nameServerRepository.isDanaAddressAvailable(username);
  ...
  Future<void> _registerUsername(BuildContext context) async {
    ...
    try {
      ...
      if (response.danaAddress != null && response.spAddress != null) {
        ...
        Logger().e(
            'Response: danaAddress=${response.danaAddress}, spAddress=${response.spAddress}');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
