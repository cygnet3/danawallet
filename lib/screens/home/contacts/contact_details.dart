...
class _ContactDetailsScreenState extends State<ContactDetailsScreen> {
  ...
  Future<void> _loadCustomFields() async {
    if (_currentContact.id != null) {
      final fields = await ContactsRepository.instance.getContactFields(_currentContact.id!);
      if (mounted) {
        setState(() {
  ...
  Future<void> _reloadContact() async {
    if (_currentContact.id != null) {
      final updatedContact = await ContactsRepository.instance.getContact(_currentContact.id!);
  ...
  Future<void> _deleteField(ContactField field) async {
    ...
    if (confirmed == true) {
      try {
        await ContactsRepository.instance.deleteContactField(field.id!);
        if (mounted) {
          await _loadCustomFields();
          setState(() {
            _contactWasUpdated = true;
