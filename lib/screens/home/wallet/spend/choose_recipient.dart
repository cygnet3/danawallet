    ...
    try {
      ...
      if (address.contains('@')) {
        ...
        try {
          ...
          Logger().d(
              'DNS resolution successful. Silent payment address: ${data.silentpayment != null ? "present" : "null"}');

          if (data.silentpayment == null || data.silentpayment!.isEmpty) {
            // DNS record exists but has no silent payment address
            Logger().w(
                'Dana address "$address" found but has no silent payment address');
            throw Exception(
                'Dana address found but has no payment address configured');
          }
