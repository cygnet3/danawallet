...
class NameServerRepository {
  ...
  String? _cleanDanaAddress(String? address) {
    ...
    if (!validAddressPattern.hasMatch(cleaned)) {
      Logger().w(
          'Dana address format may be invalid after cleaning: "$cleaned" (original: "$address")');
  ...
  Future<DanaAddressCreationResponse> registerDanaAddress({
    ...
    final requestId =
        _generateUniqueId(); // Generate ID once for all code paths
    ...
    try {
      ...
      if (data == null) {
        ...
        Logger()
            .i('Address $danaAddress not found, proceeding with registration');
      } else if (data.silentpayment != null &&
          data.silentpayment == spAddress) {
        ...
        Logger().w(
            'Found an entry for $danaAddress without a silent payment address');
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        try {
          ...
          if (responseData.danaAddress == null ||
              responseData.spAddress == null) {
            Logger().e(
                'Server returned success but missing required fields. Response: ${response.body}');
            return DanaAddressCreationResponse(
              id: requestId,
              message:
                  'Server response missing required fields: danaAddress=${responseData.danaAddress}, spAddress=${responseData.spAddress}',
              danaAddress: danaAddress,
              spAddress: spAddress,
            );
          }
          ...
          Logger().e(
              'Failed to parse successful response (${response.statusCode}): $e');
        ...
        try {
          ...
          if (response.statusCode == 404) {
            errorMessage =
                'Registration endpoint not found (404). Check API version and endpoint path.';
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        try {
          ...
          if (decoded is Map<String, dynamic> &&
              decoded.containsKey('dana_address')) {
          ...
          final validAddressPattern = RegExp(
          ...
          final validAddresses = addresses.where((addr) {
            ...
            if (!isValid) {
              Logger().w(
                  'Invalid dana address format returned from lookup: "$addr"');
            }
            return isValid;
          }).toList();
          ...
          Logger().e(
              'Failed to parse lookup response (${response.statusCode}): $e');
          Logger().e('Response body: ${response.body}');
          Logger().e('Stack trace: $stackTrace');
        ...
        // Server returned an error status
        Logger().e(
            'Lookup failed with HTTP ${response.statusCode} ${response.reasonPhrase ?? "Unknown"}');
  ...
  Future<Bip353DnsResolveResponse?> getAddressResolve(String address) async {
    ...
    if (cleanedAddress == null || cleanedAddress.isEmpty) {
      throw ArgumentError(
          "Invalid address: address is null or empty after cleaning");
    }
