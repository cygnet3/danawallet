...
impl From<OutputSpendStatus> for ApiOutputSpendStatus {
    fn from(value: OutputSpendStatus) -> Self {
        match value {
            ...
            OutputSpendStatus::Spent(txid) => ApiOutputSpendStatus::Spent(txid.to_lower_hex_string()),
            OutputSpendStatus::Mined(block) => ApiOutputSpendStatus::Mined(block.to_lower_hex_string()),
        }
    }
}
...
impl From<ApiOutputSpendStatus> for OutputSpendStatus {
    fn from(value: ApiOutputSpendStatus) -> Self {
        match value {
            ...
            ApiOutputSpendStatus::Spent(txid) => OutputSpendStatus::Spent(hex::FromHex::from_hex(&txid).unwrap()),
            ApiOutputSpendStatus::Mined(block) => OutputSpendStatus::Mined(hex::FromHex::from_hex(&block).unwrap()),
        }
    }
}
