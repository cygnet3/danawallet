...
impl OwnedOutputs {
    ...
    fn mark_mined(&mut self, outpoint: OutPoint, mined_in_block: BlockHash) -> Result<()> {
        ...
        output.spend_status = OutputSpendStatus::Mined(mined_in_block.as_raw_hash().to_byte_array());
        Ok(())
    }
    ...
    fn mark_spent(
        ...
        match &output.spend_status {
            OutputSpendStatus::Unspent => {
                output.spend_status = OutputSpendStatus::Spent(spending_tx.as_raw_hash().to_byte_array());
